...
* **Floating work widget** — 140×180 px always-on-top companion that lives on your desktop during focus sessions
...
│   ├── app.js                   # Thin wiring entry point (~130 lines)
...
│   │   ├── app-logic.test.js    # 41 tests — core app logic
...
│  app.js ──► Wiring: phase handlers, sand delegation      │
...
| Work     | 140 × 180   | Bottom-right          | Always-on-top, floating       |
...
2. Calculates raw `progress` (0 → 1) from elapsed time
...
* `backdrop-filter: blur(60px) saturate(1.5) brightness(1.04)` on glass surfaces
* Gold radial glow behind the work hourglass, pulsing on a 6 s ease-in-out cycle
...
|------------|-------------|-------|------------------------------|
| `renderer` | jsdom       | 77    | Functions, state transitions |
